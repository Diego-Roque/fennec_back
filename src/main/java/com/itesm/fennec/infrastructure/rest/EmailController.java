...
public class EmailController {
    ...
    public Response sendEmail(SendEmailDTO sendEmailDTO) {
        try {
            ...
            String email = firebaseUserService.getEmailByFirebaseId(sendEmailDTO.getFirebaseUID());
            sendEmailDTO.setFirebaseUID(email);
            sendEmailUseCase.execute(sendEmailDTO);

            return Response.ok().entity("Email sent successfully to " + email).build();
        } catch (Exception e) {
